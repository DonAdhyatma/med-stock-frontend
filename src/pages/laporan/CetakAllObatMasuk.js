...
function CetakAllObatMasuk() {
    const [data, setData] = useState([]);

    const componentRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => componentRef.current,
        documentTitle: 'Laporan Obat Masuk',
    ...
    const fetchData = async () => {
        await axios.get('http://localhost:8081/cetakallobatmasuk')
            .then(res => {
                setData(res.data);
            })
...
export default CetakAllObatMasuk;
